...
function handleOperator(op) {
  ...
  document.querySelectorAll('.btn.op').forEach(b => {
    if (b.textContent.trim() === sym2) b.classList.add('active');
  });
...
function pressEquals() {
  ...
  const sym = {'+':'+', '-':'−', '*':'×', '/':'÷'}[operator];
  exprEl.textContent = niceNum(firstOperand) + ' ' + sym + ' ' + niceNum(second) + ' =';
...
document.getElementById('btn-eq').addEventListener('click',    e => { ripple(e.currentTarget,e); pressEquals(); });
